...
class GmailParser:
    ...
    def calculate_mark(self, data):
        ...
        msg_body = self.get_msg_body(msg).split(' ')
        if len(msg_body) != 2 or not msg_body[1].isdigit():
            print("Invalid message format",msg_body)
            return 'unknown',0,received_datetime
        ...
        return contestant_name, contestant_mark, received_datetime
        ...
        with open("quiz_answers"+"_"+cur_time+".txt", "w") as file_out:
            for num in data[0].split():
                ...
                if rv != 'OK':
                    ...
                    return
                sequence_id = int(num)
                contestant_name, contestant_mark, received_time = self.calculate_mark(data)
                sender = self.calculate_received_from(data)
                file_out.write("%d\t%d\t%s\t%s\t%d\t%s\n" % (current_batch_id, sequence_id, sender, contestant_name, contestant_mark, received_time))
                query = "insert into manca.round1 values(%d,%d,'%s','%s',%d,'%s')" % (current_batch_id, sequence_id, sender, contestant_name, contestant_mark, received_time)
                # print(query)
